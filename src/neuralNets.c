                ...
                int nrow, int ncol, double penalty, double cost_hist[], int transformation_type)
{
  ...
  double time0[core_num], time1[core_num];
  ...
  for (int i = 0; i < num_iterations; i++){
    p.total_cost = 0.0;
    BatchUpdate(train_data, ys, &p, time0, time1, &time2);
  ...
  /*
  double avgtime0 = 0, avgtime1 = 0;
  for(int i = 0; i < core_num; i++){
    avgtime0 += time0[i];
    avgtime1 += time1[i];
  }
  avgtime0 /= (num_iterations * core_num);
  avgtime1 /= (num_iterations * core_num);
  time2 /= num_iterations;
  double totaltime = get_elapsed_mcycles();
  printf("\tAverage number of clock cycles\t Forward:\t %1.3f\n", avgtime0);
  printf("\tAverage number of clock cycles\t Backward:\t %1.3f\n", avgtime1);
  printf("\tAverage number of clock cycles\t Update:\t %1.3f\n", time2);
  printf("\tTotal number of clock cycles:\t\t %1.3f\n", totaltime);
  */
}
...
void BatchUpdate(gsl_vector* train_data[], gsl_vector* ys, par* p, double* t0, double* t1, double* t2){
  ...
  #pragma omp parallel
  {
    ...
    for (int j = 0; j < p->batch_size; j++){
      ...
      // Forward pass
      temp1 = get_elapsed_mcycles();
      forward(&q, p);
      temp2 = get_elapsed_mcycles();
      t0[i] += temp2 - temp1;
  ...
  temp1 = get_elapsed_mcycles();
  ispc_update(p, bias_updates, weight_updates, learning_rate);
  temp2 = get_elapsed_mcycles();
  *t2 += temp2 - temp1;
