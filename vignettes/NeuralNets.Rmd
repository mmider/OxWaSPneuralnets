...
fit_neural_network is used  for different purposes. First of all it takes in input both train and test data sets and it sets the neural net parameters: number of layers and number of neurons for each layer. Moreover the user can specify the parameters adopted in the training of the neural net: the step size of the batch gradient descent step, the regularization parameter $\lambda$. 
...
CV_neural_network improves the neural network's performance implementing a cross validation for the regularization parameter $\lambda$.
